...
func TestRunExecsUpsertSelf(t *testing.T) {
	runner := &fakeRunner{}
	mkdir := &fakeMkdirAll{}

	err := Run(context.Background(), []string{"--force"}, Dependencies{
		Runner:   runner,
	...
	if err != nil {
		t.Fatalf("run upsert-self: %v", err)
	}
	...
	if got, want := runner.args, []string{"otter-upsert-self", "--force"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("args = %#v, want %#v", got, want)
	}
	if got, want := runner.env, []string{"PATH=/bin", "HOME=/root", "AS=otter-upsert-self"}; !reflect.DeepEqual(got, want) {
...
func TestRunIgnoresMkdirError(t *testing.T) {
	...
	if got, want := runner.args, []string{"otter-upsert-self"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("args = %#v, want %#v", got, want)
	}
}
