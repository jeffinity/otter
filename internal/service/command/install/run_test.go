...
func TestRunExecsInstall(t *testing.T) {
	runner := &fakeRunner{}

	err := Run(context.Background(), []string{"--force", "api"}, Dependencies{
		Runner:     runner,
	...
	if got, want := runner.args, []string{"otter-install", "--force", "api"}; !reflect.DeepEqual(got, want) {
