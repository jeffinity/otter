...
- 根命令无子命令时默认分发到 `status` 子命令解析，支持 `status` 的全部 flags。
...
1. 命令层要求至少一个 service 参数，允许空格传入多个 service。
2. 按参数顺序逐个执行单 service 接管逻辑，任一 service 失败时立即返回错误。
3. 单 service 接管时校验 unit 存在。
4. 不改写原始 service 主体。
5. 通过 drop-in 或 otter metadata 将其纳入管理。

### `unlink-service`

取消接管已有 systemd service：

1. 输入名按 service 名规范化，允许带 `.service` 后缀。
2. 仅检查 classic 管理路径下的 `<name>.service`。
3. 只有当目标是 symlink 时才允许删除，避免误删 otter 自己安装的 classic service 文件。
4. 删除 symlink 后，原始 systemd unit 保持不变，系统服务定义不被改写。
